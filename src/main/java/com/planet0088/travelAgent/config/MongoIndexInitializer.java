...
public class MongoIndexInitializer implements CommandLineRunner {
    ...
    private void ensureHumanTasksIndexes() {
        mongoTemplate.indexOps("human_tasks").ensureIndex(
            new Index()
                .on("tenantId", Sort.Direction.ASC)
                .on("status", Sort.Direction.ASC)
                .named("idx_human_tasks_tenantId_status")
        );
        mongoTemplate.indexOps("human_tasks").ensureIndex(
            new Index()
                .on("tenantId", Sort.Direction.ASC)
                .on("bookingId", Sort.Direction.ASC)
                .named("idx_human_tasks_tenantId_bookingId")
        );
        log.debug("Ensured indexes on human_tasks(tenantId, status) and (tenantId, bookingId)");
    }
}
